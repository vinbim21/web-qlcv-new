    ...
    <FilterTable
      ...
      onAdd={() => {
        ...
        setRecord({
          title: "Thêm hạng mục",
          fields: itemFields,
          initial: { groupId: projectGroups[0]?.id ?? "", constructionTypeId: "", name: "", scale: "" },
          submit: (v) =>
            saveCatalogProject({
              groupId: v.groupId,
              name: v.name,
...
// ===================================================================
//  FilterTable — bảng phẳng có lọc theo cột + sort
// ===================================================================
function FilterTable({
  ...
  const filtered = React.useMemo(() => {
    ...
    return out;
  }, [rows, columns, colFilters, sort, colByKey]);
  ...
  const chipText = (c: Col): string => {
    ...
    if (c.filter === "multi") {
      ...
      return arr.length === 1 ? arr[0] : `${arr.length} mục`;
    }
    return `"${v as string}"`;
  };
    ...
    <div className="rounded-xl border border-slate-200 bg-card shadow-sm">
      ...
      {activeCols.length ? (
        <div className="flex flex-wrap items-center gap-1.5 px-4 pt-3">
          ...
          {activeCols.map((c) => (
            <span
              key={c.key}
              className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white py-1 pl-2.5 pr-1 text-xs shadow-sm"
            >
                      ...
                      {c.filter ? (
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            setOpenFilter((o) => (o && o.key === c.key ? null : { key: c.key, rect }));
                          }}
                          className={cn(
                            "grid size-5 place-items-center rounded",
                            on ? "bg-slate-800 text-white" : "text-slate-400 hover:bg-slate-200",
                          )}
                        >
      ...
      {openFilter && openCol ? (
        <FilterPopover
          ...
          options={multiOpts.get(openCol.key) ?? []}
...
// ===================================================================
//  ManageProjectsModal — quản lý danh sách Dự án (cấp cha)
// ===================================================================
function ManageProjectsModal({
  ...
  const [q, setQ] = React.useState("");
  const shown = groups.filter((g) => norm(g.name).includes(norm(q)) || norm(g.code).includes(norm(q)));
  return (
        ...
        <div className="max-h-[52vh] divide-y divide-slate-100 overflow-y-auto rounded-lg border border-slate-200">
          {shown.map((g) => (
            <div key={g.id} className="flex items-center gap-2 px-3 py-2">
              <Code>{g.code}</Code>
              <span className="min-w-0 flex-1 truncate font-medium text-slate-800">{g.name}</span>
              <span className="whitespace-nowrap rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-500">
                {g.itemCount} hạng mục
              </span>
              <button
                type="button"
                title="Sửa"
